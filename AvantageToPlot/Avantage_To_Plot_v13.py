###############################################################################
#                                                                             #
# Avantage_TO_Plot: Automated plotting program for the K alpha XPS system     #
#                                                                             #
# Developed by Gwilherm Kerherve at Imperial College London                   #
# (c) 2018                                                                    #
#                                                                             #
###############################################################################
...
root = Tk.Tk()
root.wm_title("Avantage To ---> PPTx")
...
fig_survey2 = plt.figure(3, figsize=(12,6))
fig_survey2 = plt.figure(3, figsize=(9.3,5))
fig_val = plt.figure(6, figsize=(4,4))
fig_fit = plt.figure(8, figsize=(14,7))    #WEIRD
...
def _view():
    plotWin = FigWin.add_subplot(111) #WEIRD
    
    #plt.yaxis.set_visible(YaxisVis) 
    #plt.ylabel("")
    #plotWin.xlabel("Binding Energy (eV)")
    #plotWin.tight_layout()
    plotWin.text(0.01,.92,'Sr3d', horizontalalignment = "left", fontsize=16, fontweight='bold'  )                                                
    #plotWin.axis([X_Max, X_Min, I_min-(I_max-I_min)*0.05+OffsetSurvey*int(len(TmpDef1.columns)/2-1), I_max*(1)*1.25])   
    #plotWin.plot(TmpDef1[tmplist[2*i]],TmpDef1[tmplist[2*i+1]]+OffsetSurvey*i, label= SmpName[i])  #WEIRD
    
    #if isChangeLocation == True : plotWin.legend(loc=Location, bbox_to_anchor=(CompOffsetX, CompOffsetY),frameon=False)
    #else : plotWin.legend(loc='upper right', bbox_to_anchor=(1, 1),frameon=False)                    

'''
def _run():
    try:
       thread.start_new_thread( run2(), ("Thread-1", 2, ) )
...
def _run():
    ...
    for tmpfile in os.listdir('.'):    
        if tmpfile.endswith(".xlsx"):
            if tmpfile.startswith("ConfigXPS_13"):
                Config_filename = tmpfile
    ...
    for i in range(0,len(Main_Settings)):
        if Main_Settings.iloc[i,0] == "Config Tab Number":
            i_TabSetting = Main_Settings.iloc[i,1]
            print('Config Tab to use: ', i_TabSetting)
        elif Main_Settings.iloc[i,0] == "Folder":
            AnaFolder = Main_Settings.iloc[i,1+i_TabSetting]
            #if AnaFolder.endswith("\\"): a = 1
            #else: AnaFolder = AnaFolder + '\\'
            AnaFolder2 = Main_Settings.iloc[i+1,1+i_TabSetting]
            if AnaFolder2.endswith("\\"): a = 1
            else: AnaFolder2 = AnaFolder2 + '\\'
            AnaFolder = AnaFolder + AnaFolder2
            print('Folder name: ', AnaFolder)
            #InfoWin.insert(Tk.END, 'Folder name: '+ AnaFolder+'\n')
        elif Main_Settings.iloc[i,0] == 'Files' :
            file =  Main_Settings.iloc[i,1+i_TabSetting].replace("\n","").replace("\t","").replace(" ","").split(",")            
            print('File: ', file)
            #InfoWin.insert(Tk.END, 'File: '+str(file)+'\n')
        elif Main_Settings.iloc[i,0] == 'Peak line & value' :        
            plotline = Main_Settings.iloc[i,1+i_TabSetting]
            plotlineS = plotline
            plotlineVal = plotline
        elif Main_Settings.iloc[i,0] == 'Visible Y values' :
            YaxisVis = Main_Settings.iloc[i,1+i_TabSetting]
        elif Main_Settings.iloc[i,0] == 'Automatic Shirley' :
            AutoShirley = False
            AutoShirley = Main_Settings.iloc[i,1+i_TabSetting]
        elif Main_Settings.iloc[i,0] == 'Automatic Survey Labelling' :
            AutoSurveyLabel = False
            AutoSurveyLabel = Main_Settings.iloc[i,1+i_TabSetting]
        elif Main_Settings.iloc[i,0] == 'Do quantification' :
            isQuant1 = Main_Settings.iloc[i,1+i_TabSetting]
        elif Main_Settings.iloc[i,0] == 'Do C-C correction' :
            IsCarbonComp = Main_Settings.iloc[i,1+i_TabSetting]
        elif Main_Settings.iloc[i,0] == 'Auto height comp' :
            IsHeightAuto = Main_Settings.iloc[i,1+i_TabSetting]
        elif Main_Settings.iloc[i,0] == 'Manual Height comp' :
            HeightComp = Main_Settings.iloc[i,1+i_TabSetting]
        elif Main_Settings.iloc[i,0] == 'Show Figure in Python' :
            UseSourcePython = Main_Settings.iloc[i,1+i_TabSetting]
        elif Main_Settings.iloc[i,0] == 'Do normalisation by val' :
            Div_Area_Check = Main_Settings.iloc[i,1+i_TabSetting]
        elif Main_Settings.iloc[i,0] == 'Normalisation value' :
            tmpDiv_Area_Val = Main_Settings.iloc[i,1+i_TabSetting].replace("\n","").replace("\t","").replace(" ","").split(",")  
            for i2 in range(0,len(file)) : 
                if tmpDiv_Area_Val[i2] == "file":
                    xl = pd.ExcelFile(AnaFolder+file[i2])
                    File_Title = xl.parse('Titles')
                    if(File_Title.iloc[2,0] == 'Normalisation'): Div_Area_Val[i2] = File_Title.iloc[2,1]
                    else : Div_Area_Val[i2] = 1
                else : Div_Area_Val[i2] = int(tmpDiv_Area_Val[i2])
        elif Main_Settings.iloc[i,0] == 'Save Images' :
            IsSaveFig = Main_Settings.iloc[i,1+i_TabSetting]
        elif Main_Settings.iloc[i,0] == 'DPI level' :
            dpiVal = Main_Settings.iloc[i,1+i_TabSetting]
        elif Main_Settings.iloc[i,0] == "Do a presentation":
            IsPresentation = Main_Settings.iloc[i,1+i_TabSetting]    
        elif Main_Settings.iloc[i,0] == "Plot width (cm)":
            Pres_plotWidth = Main_Settings.iloc[i,1+i_TabSetting]   
        elif Main_Settings.iloc[i,0] == "Filename":
            P_Fname = Main_Settings.iloc[i,1+i_TabSetting]    
        elif Main_Settings.iloc[i,0] == "Title":
            P_Title = Main_Settings.iloc[i,1+i_TabSetting]    
        elif Main_Settings.iloc[i,0] == "Template Filename":
            P_Template = Main_Settings.iloc[i,1+i_TabSetting]
        elif Main_Settings.iloc[i,0] == "Change Legend default location?":
            isChangeLocation = Main_Settings.iloc[i,1+i_TabSetting]
        elif Main_Settings.iloc[i,0] == "Legend position":
            Location = Main_Settings.iloc[i,1+i_TabSetting]
        elif Main_Settings.iloc[i,0] == "Legend Offset X":
            CompOffsetX = float(Main_Settings.iloc[i,1+i_TabSetting])
        elif Main_Settings.iloc[i,0] == "Legend Offset Y":
            CompOffsetY = float(Main_Settings.iloc[i,1+i_TabSetting])        
        elif Main_Settings.iloc[i,0] == "Autoheight for labelling":
            LabellingAutoHeight = Main_Settings.iloc[i,1+i_TabSetting]
        elif Main_Settings.iloc[i,0] == "File number for labelling":
            LabellingNumber = int(Main_Settings.iloc[i,1+i_TabSetting]) - 1
            if LabellingNumber > len(file)-1: LabellingNumber = 0
        elif Main_Settings.iloc[i,0] == "Plot settings sample number":
            PlotSettingNumber = int(Main_Settings.iloc[i,1+i_TabSetting]) - 1  
            print('Setting Number: ', PlotSettingNumber+1)
        elif Main_Settings.iloc[i,0] == "Lines settings sample number":
            LineSettingNumber = int(Main_Settings.iloc[i,1+i_TabSetting]) - 1 
            print('Setting Number: ', LineSettingNumber+1)
        elif Main_Settings.iloc[i,0] == "Offset plots":
            UseOffsetComp = Main_Settings.iloc[i,1+i_TabSetting]    
        elif Main_Settings.iloc[i,0] == "Offset Gap in Percentage":
            OffsetPercent = Main_Settings.iloc[i,1+i_TabSetting]    
        elif Main_Settings.iloc[i,0] == "Files per offset":
            OffsetFile = Main_Settings.iloc[i,1+i_TabSetting]         
        elif Main_Settings.iloc[i,0] == "Offset Maximum Percentage":
            OffsetPercent2 = Main_Settings.iloc[i,1+i_TabSetting] 
        elif Main_Settings.iloc[i,0] == "Offset in Survey":
            OffsetSurvey2 = Main_Settings.iloc[i,1+i_TabSetting]
    #MAIN CONFIG******************************************************************
    
    #AUTO SURVEY LABELLING********************************************************
    Survey_Labels = Config_file.parse('Survey Labels')
    #if AutoSurveyLabel == True:        
    #AUTO SURVEY LABELLING********************************************************
    ...
    for i in range(2,len(Line_Settings)):    
        if np.isnan(float(Line_Settings.iloc[i,0+LineSettingNumber*4])) == True: 
            print('Not a number: ', Line_Settings.iloc[i,1+LineSettingNumber*4])
            break
        #print(i, Line_Settings.iloc[i,1+LineSettingNumber*4])
        if Line_Settings.iloc[i,3+LineSettingNumber*4]:        
            DEF_lines.extend([Line_Settings.iloc[i,0+LineSettingNumber*4], Line_Settings.iloc[i,1+LineSettingNumber*4], 
                              Line_Settings.iloc[i,2+LineSettingNumber*4]])
    #END LINE SETTINGS************************************************************
    ...
    for i in range(0,len(Plot_Settings)):
        if Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'Au4f' :
            MinMaxAu4f = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinAu4f = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxAu4f = Plot_Settings.iloc[i,2+PlotSettingNumber*4]

        if Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'Ir4f' :
            MinMaxIr4f = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinIr4f = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxIr4f = Plot_Settings.iloc[i,2+PlotSettingNumber*4]
    
        if Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'Pb4f' :
            MinMaxPb4f = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinPb4f = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxPb4f = Plot_Settings.iloc[i,2+PlotSettingNumber*4]
            
        elif Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'Ce4d' :
            MinMaxCe4d = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinCe4d = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxCe4d = Plot_Settings.iloc[i,2+PlotSettingNumber*4]
        
        elif Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'Sr3d' :
            MinMaxSr3d = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinSr3d = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxSr3d = Plot_Settings.iloc[i,2+PlotSettingNumber*4]
        
        elif Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'Ce5d' :
            MinMaxCe5d = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinCe5d = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxCe5d = Plot_Settings.iloc[i,2+PlotSettingNumber*4]
        
        elif Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'Ce3d' :
            MinMaxCe3d = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinCe3d = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxCe3d = Plot_Settings.iloc[i,2+PlotSettingNumber*4]
        
        elif Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'La5d' :
            MinMaxLa5d = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinLa5d = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxLa5d = Plot_Settings.iloc[i,2+PlotSettingNumber*4]
     
        elif Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'La4d' :
            MinMaxLa4d = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinLa4d = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxLa4d = Plot_Settings.iloc[i,2+PlotSettingNumber*4]  
        
        elif Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'La3d' :
            MinMaxLa3d = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinLa3d = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxLa3d = Plot_Settings.iloc[i,2+PlotSettingNumber*4]
        
        elif Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'Ag3d' :
            MinMaxAg3d = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinAg3d = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxAg3d = Plot_Settings.iloc[i,2+PlotSettingNumber*4]

        elif Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'I3d' :
            MinMaxI3d = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinI3d = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxI3d = Plot_Settings.iloc[i,2+PlotSettingNumber*4]

        elif Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'Br3d' :
            MinMaxBr3d = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinBr3d = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxBr3d = Plot_Settings.iloc[i,2+PlotSettingNumber*4]
        
        elif Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'Zr3d' :
            MinMaxZr3d = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinZr3d = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxZr3d = Plot_Settings.iloc[i,2+PlotSettingNumber*4]

        elif Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'Y3d' :
            MinMaxY3d = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinY3d = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxY3d = Plot_Settings.iloc[i,2+PlotSettingNumber*4]
    
        elif Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'Ru3d' :
            MinMaxRu3d = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinRu3d = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxRu3d = Plot_Settings.iloc[i,2+PlotSettingNumber*4]
        
        elif Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'Sr3p' :
            MinMaxSr3p = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinSr3p = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxSr3p = Plot_Settings.iloc[i,2+PlotSettingNumber*4]
    
        elif Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'Ru3p' :
            MinMaxRu3p = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinRu3p = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxRu3p = Plot_Settings.iloc[i,2+PlotSettingNumber*4]

        elif Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'Ti2p' :
            MinMaxTi2p = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinTi2p = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxTi2p = Plot_Settings.iloc[i,2+PlotSettingNumber*4]
    
        elif Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'Mn2p' :
            MinMaxMn2p = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinMn2p = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxMn2p = Plot_Settings.iloc[i,2+PlotSettingNumber*4]

        elif Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'Cl2p' :
            MinMaxCl2p = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinCl2p = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxCl2p = Plot_Settings.iloc[i,2+PlotSettingNumber*4]
        
        elif Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'Fe2p' :
            MinMaxFe2p = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinFe2p = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxFe2p = Plot_Settings.iloc[i,2+PlotSettingNumber*4]
        
        elif Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'Co2p' :
            MinMaxCo2p = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinCo2p = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxCo2p = Plot_Settings.iloc[i,2+PlotSettingNumber*4]
    
        elif Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'Cr2p' :
            MinMaxCr2p = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinCr2p = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxCr2p = Plot_Settings.iloc[i,2+PlotSettingNumber*4]
        
        elif Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'Ni2p' :
            MinMaxNi2p = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinNi2p = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxNi2p = Plot_Settings.iloc[i,2+PlotSettingNumber*4]

        elif Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'Zn2p' :
            MinMaxZn2p = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinZn2p = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxZn2p = Plot_Settings.iloc[i,2+PlotSettingNumber*4]
    
        elif Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'Ga2p' :
            MinMaxGa2p = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinGa2p = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxGa2p = Plot_Settings.iloc[i,2+PlotSettingNumber*4]
    
        elif Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'Ga5p' :
            MinMaxGa5p = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinGa5p = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxGa5p = Plot_Settings.iloc[i,2+PlotSettingNumber*4]
    
        elif Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'P2p' :
            MinMaxP2p = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinP2p = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxP2p = Plot_Settings.iloc[i,2+PlotSettingNumber*4]
    
        elif Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'Si2p' :
            MinMaxSi2p = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinSi2p = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxSi2p = Plot_Settings.iloc[i,2+PlotSettingNumber*4]
    
        elif Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'S2p' :
            MinMaxS2p = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinS2p = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxS2p = Plot_Settings.iloc[i,2+PlotSettingNumber*4]    
    
        elif Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'Al2p' :
            MinMaxAl2p = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinAl2p = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxAl2p = Plot_Settings.iloc[i,2+PlotSettingNumber*4]  

        elif Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'Mn3s' :
            MinMaxMn3s = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinMn3s = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxMn3s = Plot_Settings.iloc[i,2+PlotSettingNumber*4]  
        
        elif Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'O1s' :
            MinMaxO1s = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinO1s = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxO1s = Plot_Settings.iloc[i,2+PlotSettingNumber*4]
    
        elif Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'Na1s' :
            MinMaxNa1s = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinNa1s = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxNa1s = Plot_Settings.iloc[i,2+PlotSettingNumber*4]
        
        elif Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'N1s' :
            MinMaxN1s = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinN1s = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxN1s = Plot_Settings.iloc[i,2+PlotSettingNumber*4]
        
        elif Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'C1s' :
            ...
            MinF1s = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxF1s = Plot_Settings.iloc[i,2+PlotSettingNumber*4]

        elif Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'Li1s' :
            MinMaxLi1s = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            MinLi1s = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            MaxLi1s = Plot_Settings.iloc[i,2+PlotSettingNumber*4]
        
        elif Plot_Settings.iloc[i,0+PlotSettingNumber*4] == 'Valence' :
            MinMaxval = Plot_Settings.iloc[i,3+PlotSettingNumber*4]
            Minval = Plot_Settings.iloc[i,1+PlotSettingNumber*4]
            Maxval = Plot_Settings.iloc[i,2+PlotSettingNumber*4]
    #PLOT SETTINGS****************************************************************
    ...
    #-----NOT SURE-------------------------------------------
    def gk_idxMax(TmpList):
        TmpMax = max(TmpList)
        #print('++++++MAX', TmpMax)
        #print(TmpList)
        for i in range(0, len(TmpList)):
            if TmpList.iloc[i] == TmpMax : break
        index_max  = i
        #print('++++++IDX_MAX', i)
        return(i)
    #-----NOT SURE-------------------------------------------
    ...
    def plot_line(DEF_lines, x_BE,plotMinMax, plotMin,plotMax, TMP_y, y_An):
        ...
        if plotMinMax: 
            a=1
            for ix_Max in range (i_start2,len(TMP_y)+i_start2):
                if round(x_BE.iloc[ix_Max],2) <= plotMax: break
            for ix_Min in range (i_start2,len(TMP_y)+i_start2):
                if round(x_BE.iloc[ix_Min],2) <= plotMin: break
            ...
            if ix_Max == ix_Min: 
                ...
                I_min = min(TMP_y)
                I_max = max(TMP_y)
                print('ERROR476.... with ', plotname, '. Check that the start region and end region are correct')
            else: 
                #TMP_y = 100 * TMP_y / max(TMP_y.iloc[ix_Max:ix_Min])
                I_min = min(TMP_y.iloc[ix_Max:ix_Min])
                I_max = max(TMP_y.iloc[ix_Max:ix_Min])
        ...
        for n in range(0,len(lab_line)):
            ...
            if lab_line_x[n] < plotMax:
                if lab_line_x[n] > plotMin:               
                    for x_i in range(i_start2,len(TMP_x_BE)):
                        if TMP_x_BE.iloc[x_i]< lab_line_x[n]:
                            #print('found it    ',TMP_x_BE.iloc[x_i])
                            break
                    TMP_y_2 = TMP_y.iloc[x_i-7:x_i+7]
                    
                    
                    #New Line x_value
                    if change_line_x[n] == True :                       
                        New_X = round(x_BE.iloc[gk_idxMax(TMP_y_2)+x_i-7],2)
                        if New_X == round(x_BE.iloc[x_i],2) : print(lab_line[n], 'Same Value')
                        else : print(lab_line[n], ': ', 'Old: ', round(x_BE.iloc[x_i],2), 'eV -->New: ', New_X, 'eV')
                        lab_line_x[n] = New_X
                        x_line[3*n] = New_X
                        x_line[3*n+1] = New_X
                        x_line[3*n+2] = New_X
    ...
    def plot_multi( tmp_fig, tmp_x, tmp_y, tmp_i,plotname, plotMinMax, x_Min, x_Max ):
        ...
        if plotMinMax == True:
            for ix_Max in range (i_start2,len(tmp_y)+i_start2):
                if round(tmp_x.iloc[ix_Max],2) <= x_Max: break
            for ix_Min in range (i_start2,len(tmp_y)+i_start2):               
                if round(tmp_x.iloc[ix_Min],2) <= x_Min: break
            
            if ix_Max == ix_Min: 
                tmp_y = 100 * tmp_y / max(tmp_y)
                I_min = min(tmp_y)
                I_max = max(tmp_y)
                print('ERROR.... with ', plotname, '. Check that the start region and end region are correct')
            else: 
                tmp_y = 100 * tmp_y / max(tmp_y.iloc[ix_Max:ix_Min])
                I_min = min(tmp_y.iloc[ix_Max:ix_Min])
                I_max = max(tmp_y.iloc[ix_Max:ix_Min])
    ...
    def plot_comp(TmpDef1, SmpName,TmpNumber,TmpName,UseMinMax,TmpMin,TmpMax,Tmpi):
        if len(TmpDef1.columns) > 2:  
            ...
            for i in range(0, int(len(TmpDef1.columns)/2)):
                ...
                if I_maxCOMP <  max(TmpDef1[tmplist[2*i+1]].iloc[ix_Max:ix_Min]):
                    I_maxCOMP = max(TmpDef1[tmplist[2*i+1]].iloc[ix_Max:ix_Min])
                    print("I_max: ", I_maxCOMP)
            '''
            i_OffsetFile = 0
            tmpOffsetFile = OffsetFile
            for i in range(0, int(len(TmpDef1.columns)/2)):
                tmplist = list(TmpDef1)
                if plotname == 'Ce & La 3d':
                    ...
                    for ix_Max in range (i_start2,len(TmpDef1[tmplist[2*i]])+i_start2):
                        if TmpDef1[tmplist[2*i]].iloc[ix_Max] < TmpMax: 
                            break
                    for ix_Min in range (i_start2,len(TmpDef1[tmplist[2*i]])+i_start2):
                        if TmpDef1[tmplist[2*i]].iloc[ix_Min] < TmpMin: break
                    if I_min > min(TmpDef1[tmplist[2*i+1]].iloc[ix_Max:ix_Min]):
                        I_min = min(TmpDef1[tmplist[2*i+1]].iloc[ix_Max:ix_Min])                
                    #I_max = max(TmpDef1[tmplist[2*i+1]].iloc[ix_Max:ix_Min])  
                    if I_max < max(TmpDef1[tmplist[2*i+1]]) : I_max = max(TmpDef1[tmplist[2*i+1]])
                elif UseMinMax == True:
                    X_Min = TmpMin
                    X_Max = TmpMax
                    for ix_Max in range (i_start2,len(TmpDef1[tmplist[2*i]])+i_start2):
                        if round(TmpDef1[tmplist[2*i]].iloc[ix_Max],2) <= TmpMax: break
                    for ix_Min in range (i_start2,len(TmpDef1[tmplist[2*i]])+i_start2):
                        #print('+++++++++++++ ',TmpDef1[tmplist[2*i]].iloc[1], '------- ', TmpDef1[tmplist[2*i]].iloc[TmpDef1[tmplist[2*i]]-1])
                        if round(TmpDef1[tmplist[2*i]].iloc[ix_Min],2) <= TmpMin: break
                    
                    if ix_Max == ix_Min:
                        print('ERROR680....  WITH ', TmpName ,'. Check that the start region and end region are correct')
                        if I_min > min(TmpDef1[tmplist[2*i+1]]): I_min = min(TmpDef1[tmplist[2*i+1]])                
                        if I_max <  max(TmpDef1[tmplist[2*i+1]]): I_max = max(TmpDef1[tmplist[2*i+1]])
                    else:
                        if I_min > min(TmpDef1[tmplist[2*i+1]].iloc[ix_Max:ix_Min]):
                            I_min = min(TmpDef1[tmplist[2*i+1]].iloc[ix_Max:ix_Min])                
                        if I_max <  max(TmpDef1[tmplist[2*i+1]].iloc[ix_Max:ix_Min]):
                            I_max = max(TmpDef1[tmplist[2*i+1]].iloc[ix_Max:ix_Min])
                ...
                if UseOffsetComp == True and plotname != 'Survey':
                    if i == 0: 
                        ...
                        if i == tmpOffsetFile:
                            i_OffsetFile = i_OffsetFile + 1
                            tmpOffsetFile = tmpOffsetFile + OffsetFile
                ...
                if(TmpNumber == 10): 
                    plt.plot(TmpDef1[tmplist[2*i]],TmpDef1[tmplist[2*i+1]]+OffsetSurvey*i, label= SmpName[i])               
                    
                    #plt.figure(10)
                    
                else: 
                    #plt.plot(TmpDef1[tmplist[2*i]],TmpDef1[tmplist[2*i+1]]+offset1*i, label= SmpName[i])
                    plt.plot(TmpDef1[tmplist[2*i]],TmpDef1[tmplist[2*i+1]]+offset1*i_OffsetFile, label= SmpName[i])
                ...
                if(TmpNumber ==10):                         
                    ...
                    if i == 0:                    
                        ...
                        for k in range(1, len(survey_label)+1):    
                            #if math.isnan(float(survey_position[k])): break
                    ...
                    if offset1 < 0:
                        #print("offset Made2")
                        plt.axis([X_Max, X_Min, I_min-((I_max-I_min)*0.05)+offset1*int((len(TmpDef1.columns)/2-1)/OffsetFile), I_max* HeightComp+offset2]) 
    ...
    def plot_fit(x_fit, data_fit,TmpName, TmpNumber,plotMinMax, plotMin,plotMax,i_fit ):
        if len(data_fit.columns) > 6:  
            ...
            i_c = 0
            y_fit = data_fit.iloc[i_start:,j_start+2]
            if data_fit.iloc[i_start-2,j_start+4] == 'Backgnd.': 
                y_BKG = data_fit.iloc[i_start:,j_start+4]
                i_0 = j_start+5
            else:
                i_0 = j_start+4
                for i_shirley in range(0, len(df1.columns)):
                    if df1.iloc[i_start-2,i_shirley] == 'Backgnd.':
                        y_BKG = data_fit.iloc[i_start:,i_shirley]
            ...
            if plotMinMax == True:
                for ix_Max in range (i_start2,len(y_fit)+i_start2):
                    if x_fit.iloc[ix_Max] < plotMax: break
                for ix_Min in range (i_start2,len(y_fit)+i_start2):
                    if x_fit.iloc[ix_Min] < plotMin: break
                #y_fit = 100 * y_fit / max(y_fit.iloc[ix_Max:ix_Min])
            
                if ix_Max == ix_Min: 
                    #y_fit = 100 * y_fit / max(y_fit)
                    I_min = min(y_fit)
                    I_max = max(y_fit)
                    print('ERROR.... ', ' Check that the start region and end region are correct')
                else: 
                    #y_fit = 100 * y_fit / max(y_fit.iloc[ix_Max:ix_Min])
                    I_min = min(y_fit.iloc[ix_Max:ix_Min])
                    I_max = max(y_fit.iloc[ix_Max:ix_Min])
            ...
            for i in range(i_0, int(len(data_fit.columns))):            
                label_fit = data_fit.iloc[i_start-2,i]
                if label_fit == 'Residuals': break
                y_fit = data_fit.iloc[i_start:,i]
                y_f = y_fit.values
                if label_fit == 'Envelope':
                     plt.plot(x_f,y_f, 'k')
                elif label_fit =='Backgnd.':
                    for i_yf in range(0,len(y_f)):
                        if y_f[i_yf] == 0 : 
                            y_f[i_yf] = y_d[i_yf]    
                    plt.plot(x_f,y_f, color = 'darkgray', linestyle='--')
                    y_b = y_f
                ...
                #plt.fill_between(x_f,y_f,y_b)#, interpolate =True)#,facecolor='grey', alpha=0.5, interpolate=True)
                    ...
                    #print('HHHHHHHHHHH    ',y_f)
                    for i_yf in range(0,len(y_f)): 
                        if y_f[i_yf] == 0 : 
                            y_f[i_yf] = y_d[i_yf]
                            #print('GOOOOOOOOOOOOOOT 0')
                    
                    
                    #WORK WITH THE LABELLING TO BE FULLY AUTOMATED
                    labelFitSplit1 = str.split(label_fit)
                    #print('1THIS IS   ', labelFitSplit1[0], '   ', len(labelFitSplit1), ' AAAAAAA  ', len(data_fit.columns))
                    
                    if labelFitSplit1[0].endswith(('p1','d3','f5')) : label_fit = '-'
                    
                    elif(len(labelFitSplit1)) > 1:
                        if(len(labelFitSplit1)) > 2 and labelFitSplit1[1] == "Scan":
                            if labelFitSplit1[1] == "Scan":
                                if labelFitSplit1[0].endswith(('p3','d5','f7')):
                                    if len(data_fit.columns) <10: 
                                        labelFitSplit1[1] = ""
                                        labelFitSplit1[2] = ""
                                    else: 
                            ...
                            #print(labelFitSplit1[0],'BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB')
                            labelFitSplit1[0] = ''
                            labelFitSplit5 = ''
                    ...
                    if labelFitSplit1[0].endswith(('1s','p3','d5','f7')): 
                        #print(labelFitSplit1[0],'AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA')
                        ...
                        labelFitSplit5 =''
                        for i1 in range(1, len(labelFitSplit1)): labelFitSplit5 = labelFitSplit5 + ' ' + labelFitSplit1[i1]
                        labelFitSplit5 = labelFitSplit4 + labelFitSplit5
                        label_fit = labelFitSplit5
                        #print('FINAL2      ',labelFitSplit5)
                
                
                    
                    elif label_fit == '-': i_c=i_c-1
    ...
    def plot_etch(x_fit, data_fit,TmpName, TmpNumber,plotMinMax, plotMin,plotMax,i_fit ):
        if len(data_fit.columns) > 6:  
            x_fit = data_fit.iloc[i_start:,j_start+0]
            y_fit = data_fit.iloc[i_start:,j_start+2]
            X_Min = x_fit.min()
            X_Max = x_fit.max()
            I_min = y_fit.min()
    ...
    df_Ti2p = pd.DataFrame()
    df_Mn2p = pd.DataFrame()
    df_Cl2p = pd.DataFrame()
    ...
    i_fit =0
    i_start = 14
    i_start2 = 0  # i_start2 exists due to ix being replace iloc & both does not take the whole array 
    j_start = 0
    ...
    for i in range(1,14) : plt.figure(i).clf()
    
    #Presentation
    if IsSaveFig == True:
        try:
            fileopen  = open(AnaFolder+P_Fname)
            fileopen.close()
        
        except:
            a=1
        
        #    print(P_Fname+' PPT file already opened by another process. YOU NEED TO CLOSE IT ..... 1')
        #    P_Fname = P_Fname+str(localtime)
        #    #else:
        #    print(P_Fname+' PPT file already opened by another process. YOU NEED TO CLOSE IT ..... 2')
        #    t = datetime.date.today()
        #    now = datetime.datetime.now()            
        #    P_Fname = str(now.year)+str(now.month)+str(now.day)+'_'+str(now.hour)+str(now.minute)+'_'+P_Fname
    ...
    for j in range(0, len(file)):
        ...
        #Sample name 
        File_Title = xl.parse('Titles')
        Sample_name.append(File_Title.iloc[3,0])
        print('Sample name: '+ File_Title.iloc[3,0])
        #InfoWin.insert(Tk.END, 'Sample name: '+ File_Title.iloc[3,0]+'\n')
        
        if(File_Title.iloc[4,0] == 'Carbon BE'): CarbonComp[j] = File_Title.iloc[4,1]
        ...
        if isQuant1 == True:
            ...
            if ['Quantifications' in xl.sheet_names] : 
                ...
                Peak_Name = File_Quant.iloc[:,File_Quant.columns.get_loc('Name ')]
                Peak_Name = Peak_Name[Peak_Name.notnull()]
                Area_Conc = File_Quant.iloc[0:len(Peak_Name)-0,File_Quant.columns.get_loc('Atomic %')]
                Peak_BE = File_Quant.iloc[:,File_Quant.columns.get_loc('Peak BE')]
                #Quant_Tab1 = File_Quant.iloc[0:len(Peak_Name)-1,[File_Quant.columns.get_loc('Name '),
                                           #File_Quant.iloc[0:len(Peak_Name)-1,File_Quant.columns.get_loc('Peak BE')], File_Quant.columns.get_loc('FWHM eV'), File_Quant.columns.get_loc('Atomic %')]]
                Quant_Tab1 = File_Quant.iloc[0:len(Peak_Name)-0,[File_Quant.columns.get_loc('Name '),
                ...
                for j_tab in range(0, len(Peak_Name)):
                    Quant_Tab1.iloc[j_tab,1] = round(Quant_Tab1.iloc[j_tab,1] - CarbonComp[j]+284.8,2)
                print(Quant_Tab1)
                ...
                if Column_Num1 > 6:
                    ...
                    Quant_Tab2 = File_Quant.iloc[0:len(Peak_Name)-0, File_Quant.columns.get_loc('Atomic %')+1:]
                    #print(Quant_Tab2)
                    Header_Tab2 = list(Quant_Tab2)
                    Quant_Tab2_Check = Quant_Tab2.notnull()
                    SOS_Tab2_Check = Quant_Tab2.isin(['0'])
                    #print('ABABABABABABABA      ', SOS_Tab2_Check )
                    ...
                    for i_tab in range(0 , len(Header_Tab2)):
                        list1 = list(Peak_Name[Quant_Tab2_Check.iloc[:,i_tab]])
                        #print('xxxxxxxx ', list1[0], '    ', i_tab)
                        word1[i_tab+1] = list1[0]
                        
                        for j_tab in range(1,len(list1)) : word1[i_tab+1] = word1[i_tab+1]+' : '+list1[j_tab]
                        #print('xxxxxx ', word1[i_tab+1])
                        #Quant_Tab3.iloc[i_tab+1,0] = word1                        
                        
                        list2 = list(Area_Conc[Quant_Tab2_Check.iloc[:,i_tab]])
                        list3 = list(Area_Conc[Quant_Tab2_Check.iloc[:,i_tab]])
                        #print('ABABABABABBABAB  ', list3)
                        #print('zzzzzz ', list2, 'xxxx ', list2 / sum(list2[:]))
                        #list2 = np.round(list2 / sum(list2[:]) *100)
                        for i_list in range (0,len(list2)): 
                            list3[i_list] = np.round(list2[i_list] / sum(list2[:]) *100)
                        #print(list3)
                        word2[i_tab+1] = str(list3[0])                    
                        for j_tab in range(1,len(list3)) : word2[i_tab+1] = word2[i_tab+1]+' : '+str(list3[j_tab])
                        #print(word2)
                    ...
                    for i_tab in range(0, len(Header_Tab2)+1): print(Quant_Tab3[i_tab][0] ,'       | ' , Quant_Tab3[i_tab][1])
                else: 
                    Quant_Tab2 = File_Quant.iloc[0:len(Peak_Name)-0, File_Quant.columns.get_loc('Atomic %')+2:]
                    Header_Tab2 = list(Quant_Tab2)
                    Quant_Tab3 = list(Quant_Tab2)
        ...
        print('---------Peak Labelling-------------------------------')
        
        plt.figure(1).clf()        #123689
        plt.figure(2).clf()
        plt.figure(3).clf() 
        plt.figure(6).clf() 
        plt.figure(8).clf()
        ...
        for i in range(0, len(xl.sheet_names)-1):    
            ...
            if plotname[0:2] == 'Sh': a=1
            elif plotname[0:2] == 'Pl': a=1
            elif plotname[0:2] == 'Ti': a=1
            elif plotname[0:2] == 'Qu': a=1
            elif plotname[0:2] == 'Su': IsSurvey = True   
            else:  IsCoreLevel = True
            ...
            for j_start in range(0, 4):
                #print(df1)
                x_BE = df1.iloc[0:,j_start]
                isJ_Start = False
                for i_start in range(0, len(x_BE)):                
                    if x_BE.iloc[i_start] == 'eV':
                        isJ_Start = True
                        i_start = i_start+1
                        break        
                if isJ_Start: 
            ...
            x_BE = df1.iloc[i_start:,j_start]
            TMPCarbonComp = 284.8 - CarbonComp[j]
            x_BE = x_BE+ TMPCarbonComp
            
            y = df1.iloc[i_start:,j_start+2]
            ...
            if i_start-9 > 0:
                checkFit = df1.iloc[i_start-9,j_start+1]
            ...
            if (AutoShirley == True) or (len(df1.columns) > j_start+4):
                if AutoShirley == True:
                ...
                elif df1.iloc[i_start-2,j_start+4] == 'Backgnd.':                
                    IsShirley = True
                    y_BKG1 = df1.iloc[i_start:,j_start+4]
                    y3 = y - y_BKG1
                    if Div_Area_Check == True: y3 = y3 / Div_Area_Val[j]
                    else:
                        y3 = y3 / max(y3)
                    ...
                    for i_shirley in range(0, len(df1.columns)):
                        if df1.iloc[i_start-2,i_shirley] == 'Backgnd.':
                            print('GOT SHIRLEY')
                            IsShirley = True
                            y_BKG1 = df1.iloc[i_start:,i_shirley]
                            y3 = y - y_BKG1
                            if Div_Area_Check == True: y3 = y3 / Div_Area_Val[j]
                            else:
                                y3 = y3 / max(y3)
            ...
            #-----CHECK Ir4F-------------------------------------
            if plotname == 'Ir 4f':
                if MinMaxIr4f == True:                                              #START HERE
                    plotMinMax = True                                   
                    for i_Max in range (i_start,len(x_BE)+i_start):
                        if x_BE[i_Max] < MaxIr4f: break#                        HERE
                    for i_Min in range (i_start,len(x_BE)+i_start):
                        if x_BE[i_Min] < MinIr4f: break#                        HERE       
                    plotMax = x_BE[i_Max]
                    plotMin = x_BE[i_Min]
                    if Div_Area_Check == False: 
                        y_BKG = (y[i_Min-3]+y[i_Min-2]+y[i_Min-1]+y[i_Min])/4       
                        ...
                        y2 = (y - y_BKG) / Div_Area_Val[j]       
                #df_Ir4f['BE'+Sample_name[j]] = x_BE
                additional = pd.DataFrame({'BE'+Sample_name[j]: x_BE, file[j]:y2})
                new = pd.concat([df_Ir4f,additional],axis =1)
                df_Ir4f = new
                df_Ir4fT.append(Sample_name[j])
                #print(df_Ir4f.head())
                #df_Ir4f[file[j]] = y2
                if (AutoShirley == True) or (len(df1.columns) > j_start + 4):                
                    if (AutoShirley == True) or (df1.iloc[i_start-2,j_start+4] == 'Backgnd.'):
                        #df_Ir4fs['BE'+Sample_name[j]] = x_BE
                        #df_Ir4fs[file[j]] = y3
                        additional = pd.DataFrame({'BE'+Sample_name[j]: x_BE, file[j]:y3})
                        new = pd.concat([df_Ir4fs,additional],axis =1)
                        df_Ir4fs = new
                        df_Ir4fsT.append(Sample_name[j])
            #----------------------------------------------------
            ...
            #-----CHECK I3D-------------------------------------
                ...
                if MinMaxI3d == True:                                              #START HERE
                    plotMinMax = True                                
                    for i_Max in range (i_start,len(x_BE)+i_start):
                        if x_BE[i_Max] < MaxI3d: break#                        HERE
                    for i_Min in range (i_start,len(x_BE)+i_start):
                        if x_BE[i_Min] < MinI3d: break#                        HERE       
                    plotMax = x_BE[i_Max]
                    plotMin = x_BE[i_Min]
                    if Div_Area_Check == False:    
                        y_BKG = (y[i_Min-3]+y[i_Min-2]+y[i_Min-1]+y[i_Min])/4       
                        ...
                        y2 = (y - y_BKG) / Div_Area_Val[j]       
                #df_I3d['BE'+Sample_name[j]] = x_BE
                #df_I3d[file[j]] = y2
                additional = pd.DataFrame({'BE'+Sample_name[j]: x_BE, file[j]:y2})
                new = pd.concat([df_I3d,additional],axis =1)
                df_I3d = new
                df_I3dT.append(Sample_name[j])
                if (AutoShirley == True) or (len(df1.columns) > j_start + 4):                
                    if (AutoShirley == True) or (df1.iloc[i_start-2,j_start+4] == 'Backgnd.'):
                        #df_I3ds['BE'+Sample_name[j]] = x_BE
                        #df_I3ds[file[j]] = y3
                        additional = pd.DataFrame({'BE'+Sample_name[j]: x_BE, file[j]:y3})
                        new = pd.concat([df_I3ds,additional],axis =1)
                        df_I3ds = new
                        df_I3dsT.append(Sample_name[j])
            #----------------------------------------------------

            #-----CHECK Br3D-------------------------------------
            elif plotname == 'Br 3d':
                if MinMaxBr3d == True:                                              #START HERE
                    plotMinMax = True                                
                    for i_Max in range (i_start,len(x_BE)+i_start):
                        if x_BE[i_Max] < MaxBr3d: break#                        HERE
                    for i_Min in range (i_start,len(x_BE)+i_start):
                        if x_BE[i_Min] < MinBr3d: break#                        HERE       
                    plotMax = x_BE[i_Max]
                    plotMin = x_BE[i_Min]
                    if Div_Area_Check == False:    
                        y_BKG = (y[i_Min-3]+y[i_Min-2]+y[i_Min-1]+y[i_Min])/4       
                        ...
                        y2 = (y - y_BKG) / Div_Area_Val[j]       
                #df_Br3d['BE'+Sample_name[j]] = x_BE
                #df_Br3d[file[j]] = y2
                additional = pd.DataFrame({'BE'+Sample_name[j]: x_BE, file[j]:y2})
                new = pd.concat([df_Br3d,additional],axis =1)
                df_Br3d = new
                df_Br3dT.append(Sample_name[j])
                if (AutoShirley == True) or (len(df1.columns) > j_start + 4):                
                    if (AutoShirley == True) or (df1.iloc[i_start-2,j_start+4] == 'Backgnd.'):
                        #df_Br3ds['BE'+Sample_name[j]] = x_BE
                        #df_Br3ds[file[j]] = y3
                        additional = pd.DataFrame({'BE'+Sample_name[j]: x_BE, file[j]:y3})
                        new = pd.concat([df_Br3ds,additional],axis =1)
                        df_Br3ds = new
                        df_Br3dsT.append(Sample_name[j])
            #----------------------------------------------------
            ...
            #-----CHECK Y3d-------------------------------------
                ...
                if MinMaxY3d == True:                                              #START HERE
                    plotMinMax = True                                   
                    for i_Max in range (i_start,len(x_BE)+i_start):
                        if x_BE[i_Max] < MaxY3d: break#                        HERE
                    for i_Min in range (i_start,len(x_BE)+i_start):
                        if x_BE[i_Min] < MinY3d: break#                        HERE       
                    plotMax = x_BE[i_Max]
                    plotMin = x_BE[i_Min]
                    if Div_Area_Check == False: 
                        y_BKG = (y[i_Min-3]+y[i_Min-2]+y[i_Min-1]+y[i_Min])/4       
                        ...
                        y2 = (y - y_BKG) / Div_Area_Val[j]       
                #df_Y3d['BE'+Sample_name[j]] = x_BE
                #df_Y3d[file[j]] = y2
                additional = pd.DataFrame({'BE'+Sample_name[j]: x_BE, file[j]:y2})
                new = pd.concat([df_Y3d,additional],axis =1)
                df_Y3d = new
                df_Y3dT.append(Sample_name[j])
                if (AutoShirley == True) or (len(df1.columns) > j_start + 4):                
                    if (AutoShirley == True) or (df1.iloc[i_start-2,j_start+4] == 'Backgnd.'):
                        #df_Y3ds['BE'+Sample_name[j]] = x_BE
                        #df_Y3ds[file[j]] = y3
                        additional = pd.DataFrame({'BE'+Sample_name[j]: x_BE, file[j]:y3})
                        new = pd.concat([df_Y3ds,additional],axis =1)
                        df_Y3ds = new
                        df_Y3dsT.append(Sample_name[j])
            #----------------------------------------------------
            ...
            #-----CHECK MN2P-------------------------------------
                ...
                if MinMaxMn2p == True:                                              #START HERE
                    plotMinMax = True                                   
                    for i_Max in range (i_start,len(x_BE)+i_start):
                        if x_BE[i_Max] < MaxMn2p: break#                        HERE
                    for i_Min in range (i_start,len(x_BE)+i_start):
                        if x_BE[i_Min] < MinMn2p: break#                        HERE       
                    plotMax = x_BE[i_Max]
                    plotMin = x_BE[i_Min]
                    if Div_Area_Check == False: 
                        y_BKG = (y[i_Min-3]+y[i_Min-2]+y[i_Min-1]+y[i_Min])/4       
                ...
                if (AutoShirley == True) or (len(df1.columns) > j_start + 4):                
                    if (AutoShirley == True) or (df1.iloc[i_start-2,j_start+4] == 'Backgnd.'):
                        #df_Mn2ps['BE'+Sample_name[j]] = x_BE
                        #df_Mn2ps[file[j]] = y3
                        additional = pd.DataFrame({'BE'+Sample_name[j]: x_BE, file[j]:y3})
            ...
            #-----CHECK TI2P-------------------------------------
                ...
                if MinMaxTi2p == True:                                              #START HERE
                    plotMinMax = True                                   
                    for i_Max in range (i_start,len(x_BE)+i_start):
                        if x_BE[i_Max] < MaxTi2p: break#                        HERE
                    for i_Min in range (i_start,len(x_BE)+i_start):
                        if x_BE[i_Min] < MinTi2p: break#                        HERE       
                    plotMax = x_BE[i_Max]
                    plotMin = x_BE[i_Min]
                    if Div_Area_Check == False: 
                        y_BKG = (y[i_Min-3]+y[i_Min-2]+y[i_Min-1]+y[i_Min])/4       
                        ...
                        y2 = (y - y_BKG) / Div_Area_Val[j]       
                #df_Ti2p['BE'+Sample_name[j]] = x_BE
                #df_Ti2p[file[j]] = y2
                additional = pd.DataFrame({'BE'+Sample_name[j]: x_BE, file[j]:y2})
                new = pd.concat([df_Ti2p,additional],axis =1)
                df_Ti2p = new
                df_Ti2pT.append(Sample_name[j])
                if (AutoShirley == True) or (len(df1.columns) > j_start + 4):                
                    if (AutoShirley == True) or (df1.iloc[i_start-2,j_start+4] == 'Backgnd.'):
                        #df_Ti2ps['BE'+Sample_name[j]] = x_BE
                        #df_Ti2ps[file[j]] = y3
                        additional = pd.DataFrame({'BE'+Sample_name[j]: x_BE, file[j]:y3})
                        new = pd.concat([df_Ti2ps,additional],axis =1)
                        df_Ti2ps = new
                        df_Ti2psT.append(Sample_name[j])
            #----------------------------------------------------

            #-----CHECK CL2P-------------------------------------
            elif plotname == 'Cl 2p':
                if MinMaxCl2p == True:                                              #START HERE
                    plotMinMax = True                                   
                    for i_Max in range (i_start,len(x_BE)+i_start):
                        if x_BE[i_Max] < MaxCl2p: break#                        HERE
                    for i_Min in range (i_start,len(x_BE)+i_start):
                        if x_BE[i_Min] < MinCl2p: break#                        HERE       
                    plotMax = x_BE[i_Max]
                    plotMin = x_BE[i_Min]
                    if Div_Area_Check == False: 
                        y_BKG = (y[i_Min-3]+y[i_Min-2]+y[i_Min-1]+y[i_Min])/4       
                        ...
                        y2 = (y - y_BKG) / Div_Area_Val[j]       
                #df_Cl2p['BE'+Sample_name[j]] = x_BE
                #df_Cl2p[file[j]] = y2
                additional = pd.DataFrame({'BE'+Sample_name[j]: x_BE, file[j]:y2})
                new = pd.concat([df_Cl2p,additional],axis =1)
                df_Cl2p = new
                df_Cl2pT.append(Sample_name[j])
                if (AutoShirley == True) or (len(df1.columns) > j_start + 4):                
                    if (AutoShirley == True) or (df1.iloc[i_start-2,j_start+4] == 'Backgnd.'):
                        #df_Cl2ps['BE'+Sample_name[j]] = x_BE
                        #df_Cl2ps[file[j]] = y3
                        additional = pd.DataFrame({'BE'+Sample_name[j]: x_BE, file[j]:y3})
                        new = pd.concat([df_Cl2ps,additional],axis =1)
                        df_Cl2ps = new
                        df_Cl2psT.append(Sample_name[j])
            #----------------------------------------------------

            #-----CHECK S2P-------------------------------------
            elif plotname == 'S 2p':
                if MinMaxS2p == True:                                              #START HERE
                    plotMinMax = True                                   
                    for i_Max in range (i_start,len(x_BE)+i_start):
                        if x_BE[i_Max] < MaxS2p: break#                        HERE
                    for i_Min in range (i_start,len(x_BE)+i_start):
                        if x_BE[i_Min] < MinS2p: break#                        HERE       
                    plotMax = x_BE[i_Max]
                    plotMin = x_BE[i_Min]
                    if Div_Area_Check == False: 
                        y_BKG = (y[i_Min-3]+y[i_Min-2]+y[i_Min-1]+y[i_Min])/4       
            ...
            #-----CHECK Al2P-------------------------------------
                ...
                if MinMaxAl2p == True:                                              #START HERE
                    plotMinMax = True                                   
                    for i_Max in range (i_start,len(x_BE)+i_start):
                        if x_BE[i_Max] < MaxAl2p: break#                        HERE
                    for i_Min in range (i_start,len(x_BE)+i_start):
                        if x_BE[i_Min] < MinAl2p: break#                        HERE       
                    plotMax = x_BE[i_Max]
                    plotMin = x_BE[i_Min]
                    if Div_Area_Check == False: 
                        y_BKG = (y[i_Min-3]+y[i_Min-2]+y[i_Min-1]+y[i_Min])/4       
                ...
                if (AutoShirley == True) or (len(df1.columns) > j_start + 4):                
                    if (AutoShirley == True) or (df1.iloc[i_start-2,j_start+4] == 'Backgnd.'):
                        #df_Al2ps['BE'+Sample_name[j]] = x_BE
                        #df_Al2ps[file[j]] = y3
                        additional = pd.DataFrame({'BE'+Sample_name[j]: x_BE, file[j]:y3})
            ...
            #-----CHECK Zn2p--------------------------------------
                ...
                #print(j,'  ',len(y3), '   ', len(x_BE))
                if MinMaxZn2p == True:                                              #START HERE
                    plotMinMax = True                                
                    for i_Max in range (i_start,len(x_BE)+i_start):
                        if x_BE[i_Max] < MaxZn2p: break#                        HERE
                    for i_Min in range (i_start,len(x_BE)+i_start):
                        if x_BE[i_Min] < MinZn2p: break#                        HERE       
                    plotMax = x_BE[i_Max]
                    plotMin = x_BE[i_Min]
                    if Div_Area_Check == False:    
                        y_BKG = (y[i_Min-3]+y[i_Min-2]+y[i_Min-1]+y[i_Min])/4       
                        ...
                        ##print('ccccccccccccccccccc    ', y[i_Min], '  ', i_Min)
                        if i_Min -3 < i_start: y_BKG = y[i_Min]
                        else: y_BKG = (y[i_Min-3]+y[i_Min-2]+y[i_Min-1]+y[i_Min])/4       
                        #print('ddddddddddddddddddd    ',y[i_Min-3])
                        
                        y2 = (y - y_BKG) / Div_Area_Val[j]       
                additional = pd.DataFrame({'BE'+Sample_name[j]: x_BE, file[j]:y2})
                new = pd.concat([df_Zn2p,additional],axis =1)
                df_Zn2p = new  
                df_Zn2pT.append(Sample_name[j])
                if (AutoShirley == True) or (len(df1.columns) > j_start + 4):                
                    if (AutoShirley == True) or (df1.iloc[i_start-2,j_start+4] == 'Backgnd.'):
                        additional = pd.DataFrame({'BE'+Sample_name[j]: x_BE, file[j]:y3})
                        new = pd.concat([df_Zn2ps,additional],axis =1)
                        df_Zn2ps = new  
                        df_Zn2psT.append(Sample_name[j])
            #----------------------------------------------------
            ...
            #-----CHECK Mn3s-------------------------------------
                ...
                if MinMaxMn3s == True:                                              #START HERE
                    plotMinMax = True                                   
                    for i_Max in range (i_start,len(x_BE)+i_start):
                        if x_BE[i_Max] < MaxMn3s: break#                        HERE
                    for i_Min in range (i_start,len(x_BE)+i_start):
                        if x_BE[i_Min] < MinMn3s: break#                        HERE       
                    plotMax = x_BE[i_Max]
                    plotMin = x_BE[i_Min]
                    if Div_Area_Check == False: 
                        y_BKG = (y[i_Min-3]+y[i_Min-2]+y[i_Min-1]+y[i_Min])/4       
                        ...
                        y2 = (y - y_BKG) / Div_Area_Val[j]       
                #df_Mn3s['BE'+Sample_name[j]] = x_BE
                #df_Mn3s[file[j]] = y2
                additional = pd.DataFrame({'BE'+Sample_name[j]: x_BE, file[j]:y2})
                new = pd.concat([df_Mn3s,additional],axis =1)
                df_Mn3s = new
                df_Mn3sT.append(Sample_name[j])
                if (AutoShirley == True) or (len(df1.columns) > j_start + 4):                
                    if (AutoShirley == True) or (df1.iloc[i_start-2,j_start+4] == 'Backgnd.'):
                        #df_Mn3ss['BE'+Sample_name[j]] = x_BE
                        #df_Mn3ss[file[j]] = y3
                        additional = pd.DataFrame({'BE'+Sample_name[j]: x_BE, file[j]:y3})
                        new = pd.concat([df_Mn3ss,additional],axis =1)
                        df_Mn3ss = new
                        df_Mn3ssT.append(Sample_name[j])
            #----------------------------------------------------
            ...
            #-----CHECK Li1s--------------------------------------
                ...
                #print(j,'  ',len(y3), '   ', len(x_BE))
                if MinMaxLi1s == True:                                              #START HERE
                    plotMinMax = True                                  
                    for i_Max in range (i_start,len(x_BE)+i_start):
                        if x_BE[i_Max] < MaxLi1s: break#                        HERE
                    for i_Min in range (i_start,len(x_BE)+i_start):
                        if x_BE[i_Min] < MinLi1s: break#                        HERE       
                    plotMax = x_BE[i_Max]
                    plotMin = x_BE[i_Min]
                    if Div_Area_Check == False:  
                        y_BKG = (y[i_Min-3]+y[i_Min-2]+y[i_Min-1]+y[i_Min])/4       
                        ...
                        y2 = (y - y_BKG) / Div_Area_Val[j]       
                additional = pd.DataFrame({'BE'+Sample_name[j]: x_BE, file[j]:y2})
                new = pd.concat([df_Li1s,additional],axis =1)
                df_Li1s = new  
                df_Li1sT.append(Sample_name[j])
                if (AutoShirley == True) or (len(df1.columns) > j_start + 4):                
                    if (AutoShirley == True) or (df1.iloc[i_start-2,j_start+4] == 'Backgnd.'):
                        additional = pd.DataFrame({'BE'+Sample_name[j]: x_BE, file[j]:y3})
                        new = pd.concat([df_Li1ss,additional],axis =1)
                        df_Li1ss = new  
                        df_Li1ssT.append(Sample_name[j])
            #----------------------------------------------------
        ...
        if len(df_survey.columns) == 2:
            ...
            #InfoWin.insert(Tk.END, 'Plot Survey'+'\n')
            ...
            if AutoSurveyLabel == False:                  
                survey_label= df1.iloc[1:,1]
                survey_position= df1.iloc[1:,3]
                #survey_BE = df_survey.iloc[i_start:,0]
                #survey_y  = df_survey.iloc[i_start:,1]
                survey_BE = df_survey.iloc[:,0]
                survey_y  = df_survey.iloc[:,1]            
                #print(survey_BE)
                i4 =10
                
                for k in range(1, len(survey_label)+1):
                    if math.isnan(float(survey_position[k])): break  # Height CPS
                    for l in range(i_start, len(survey_BE)):
                        i4 = l                    
                        if float(survey_BE[l]) < float(survey_position[k]):
                            i4 = l
            ...
            #AUTO LABEL--------------------------------------------------------
                ...
                survey_labels_tmp = df1.iloc[1:,1]
                for k in range(1, len(survey_label_tmp)+1):
                    if math.isnan(float(survey_position[k])): break  # Height CPS
                    for l in range(i_start, len(survey_BE)):
                        a=1
            #LABELS PEAK----------------------------------------------------------
            ...
            #canvas._tkcanvas.grid_remove()

            #canvas3.get_tk_widget().grid_remove()
            #canvas3._tkcanvas.grid_remove()
        ...
        if IsSaveFig == True:
            ...
            if IsSurvey == True:
                ...
                pic = slide.shapes.add_picture(AnaFolder+'PNGs\Survey'+'_'+Sample_name[j]+'.png', 0, Cm(3.12), width=Cm(Pres_plotWidth))
            ...
            if IsCoreLevel == True:
                ...
                pic = slide.shapes.add_picture(AnaFolder+'PNGs\CoreLevel'+'_'+Sample_name[j]+'.png', 0, Cm(3.12), width=Cm(Pres_plotWidth))
                 
            if IsCoreLevel == True and IsShirley == True:
                blank_slide_layout = prs.slide_layouts[5]
                slide = prs.slides.add_slide(blank_slide_layout)
                shapes = slide.shapes
                ...
                pic = slide.shapes.add_picture(AnaFolder+'PNGs\CoreLevelShirley'+'_'+Sample_name[j]+'.png', 
                                               0, Cm(3.12), width=Cm(Pres_plotWidth))
            ...
            if isQuant == True:
                ...
                for i_tab in range(0, len(Header_Tab)):
                    ...
                    for j_tab in range(0, len(Peak_Name)):
                        '''
                        if i_tab == 1:
                                Quant_Tab1.iloc[j_tab,i_tab] = Quant_Tab1.iloc[j_tab,i_tab] - CarbonComp[j]+284.8
                                '''
                        table.cell(j_tab+1, i_tab).text = str(Quant_Tab1.iloc[j_tab,i_tab])
                ...
                if rows !=0 :                    
                    ...
                    #for i_tab in range(0, len(Header_Tab2)):
                    for i_tab in range(0, len(Quant_Tab3)):
            ...
            #SHOW ON CANVAS WINDOW-------------------------------
            #plt.figure(3).clf
            #canvas2.get_tk_widget().grid_remove()
            #canvas2._tkcanvas.grid_remove()            
            
            canvas.get_tk_widget().grid()
            canvas._tkcanvas.grid()
            #InfoWin.insert(Tk.END, 'Plot Core Levels'+'\n')
            canvas.draw()
        ...
        if UseSourcePython == True: 
            ...
            plt.figure(6).clf()
            plt.figure(8).clf()
            #for i in range(1,11) : plt.figure(i).clf()
        #else:             
            #plt.figure(1).clf()
            #plt.figure(2).clf()
            #plt.figure(3).clf()
    ...
    if len(tmplist)>2 : 
        i2 =plot_comp(df_Ir4f, df_Ir4fT,4,'Ir 4f',MinMaxIr4f, MinIr4f, MaxIr4f,i2)
        df_Ir4f.to_csv(AnaFolder+'Data_CSV\comp_Ir4f.csv', sep='\t')
        if plotline == True:
            if UseOffsetComp == True: LabellingNumber = 0
            if len(tmplist)>2 : plot_line(DEF_lines, df_Ir4f[tmplist[0]], MinMaxIr4f, MinIr4f, MaxIr4f, df_Ir4f[tmplist[2*LabellingNumber+1]], 1.12)
    tmplist = list(df_Ir4fs)
    if len(tmplist)>2 : 
        if IsShirley:
            i2s =plot_comp(df_Ir4fs, df_Ir4fsT,5,'Ir 4f',MinMaxIr4f, MinIr4f, MaxIr4f,i2s)
            df_Ir4fs.to_csv(AnaFolder+'Data_CSV\comp_Ir4fS.csv', sep='\t')
            if plotline == True:
                plot_line(DEF_lines, df_Ir4fs[tmplist[0]], MinMaxIr4f, MinIr4f, MaxIr4f, df_Ir4fs[tmplist[2*LabellingNumber+1]], 1.12)
    #Ir4f---------------------------------------------------
    ...
    if len(tmplist)>2 : 
        i2 =plot_comp(df_Ag3d, df_Ag3dT,4,'Ag 3d',MinMaxAg3d, MinAg3d, MaxAg3d,i2)
        df_Ag3d.to_csv(AnaFolder+'Data_CSV\comp_Ag3d.csv', sep='\t')
        if plotline == True:
            if UseOffsetComp == True: LabellingNumber = 0
    ...
    if len(tmplist)>2 :
        if IsShirley:
            i2s =plot_comp(df_Ag3ds, df_Ag3dsT,5,'Ag 3d',MinMaxAg3d, MinAg3d, MaxAg3d,i2s)
            df_Ag3ds.to_csv(AnaFolder+'Data_CSV\comp_Ag3dS.csv', sep='\t')
            if plotline == True:
                plot_line(DEF_lines, df_Ag3ds[tmplist[0]], MinMaxAg3d, MinAg3d, MaxAg3d, df_Ag3ds[tmplist[2*LabellingNumber+1]], 1.12)
    #Ag3d---------------------------------------------------
    ...
    if len(tmplist)>2 : 
        i2 =plot_comp(df_I3d, df_I3dT,4,'I 3d',MinMaxI3d, MinI3d, MaxI3d,i2)
        df_I3d.to_csv(AnaFolder+'Data_CSV\comp_I3d.csv', sep='\t')
        if plotline == True:
            if UseOffsetComp == True: LabellingNumber = 0
            plot_line(DEF_lines, df_I3d[tmplist[0]], MinMaxI3d, MinI3d, MaxI3d, df_I3d[tmplist[2*LabellingNumber+1]], 1.12)
    tmplist = list(df_I3ds)
    if len(tmplist)>2 :
        if IsShirley:
            i2s =plot_comp(df_I3ds, df_I3dsT,5,'I 3d',MinMaxI3d, MinI3d, MaxI3d,i2s)
            df_I3ds.to_csv(AnaFolder+'Data_CSV\comp_I3dS.csv', sep='\t')
            if plotline == True:
                plot_line(DEF_lines, df_I3ds[tmplist[0]], MinMaxI3d, MinI3d, MaxI3d, df_I3ds[tmplist[2*LabellingNumber+1]], 1.12)
    #I3d--------------------------------------------------- 

    #Br3d---------------------------------------------------
    tmplist = list(df_Br3d)
    ...
    if len(tmplist)>2 : 
        i2 =plot_comp(df_Br3d, df_Br3dT,4,'Br 3d',MinMaxBr3d, MinBr3d, MaxBr3d,i2)
        df_Br3d.to_csv(AnaFolder+'Data_CSV\comp_Br3d.csv', sep='\t')
        if plotline == True:
            if UseOffsetComp == True: LabellingNumber = 0
            plot_line(DEF_lines, df_Br3d[tmplist[0]], MinMaxBr3d, MinBr3d, MaxBr3d, df_Br3d[tmplist[2*LabellingNumber+1]], 1.12)
    tmplist = list(df_Br3ds)
    if len(tmplist)>2 :
        if IsShirley:
            i2s =plot_comp(df_Br3ds, df_Br3dsT,5,'Br 3d',MinMaxBr3d, MinBr3d, MaxBr3d,i2s)
            df_Br3ds.to_csv(AnaFolder+'Data_CSV\comp_Br3dS.csv', sep='\t')
            if plotline == True:
                plot_line(DEF_lines, df_Br3ds[tmplist[0]], MinMaxBr3d, MinBr3d, MaxBr3d, df_Br3ds[tmplist[2*LabellingNumber+1]], 1.12)
    #Br3d---------------------------------------------------
    ...
    if len(tmplist)>2 : 
        i2 =plot_comp(df_Mn2p, df_Mn2pT,4,'Mn 2p',MinMaxMn2p,MinMn2p,MaxMn2p,i2)
    ...
    if len(tmplist)>2 : 
        if IsShirley:
            i2s =plot_comp(df_Mn2ps, df_Mn2psT,5,'Mn 2p',MinMaxMn2p,MinMn2p,MaxMn2p,i2s)
    ...
    if len(tmplist)>2 : 
        i2 =plot_comp(df_Ti2p, df_Ti2pT,4,'Ti 2p',MinMaxTi2p,MinTi2p,MaxTi2p,i2)
        df_Ti2p.to_csv(AnaFolder+'Data_CSV\comp_Ti2p.csv', sep='\t')
        if plotline == True:
            if UseOffsetComp == True: LabellingNumber = 0
            plot_line(DEF_lines, df_Ti2p[tmplist[0]], MinMaxTi2p, MinTi2p, MaxTi2p, df_Ti2p[tmplist[2*LabellingNumber+1]], 1.12)
    tmplist = list(df_Ti2ps)
    if len(tmplist)>2 : 
        if IsShirley:
            i2s =plot_comp(df_Ti2ps, df_Ti2psT,5,'Ti 2p',MinMaxTi2p,MinTi2p,MaxTi2p,i2s)
            df_Ti2ps.to_csv(AnaFolder+'Data_CSV\comp_Ti2pS.csv', sep='\t')
            if plotline == True:
                plot_line(DEF_lines, df_Ti2ps[tmplist[0]], MinMaxTi2p, MinTi2p, MaxTi2p, df_Ti2ps[tmplist[2*LabellingNumber+1]], 1.12)
    #Ti2p---------------------------------------------------

    #Cl2p---------------------------------------------------
    tmplist = list(df_Cl2p)
    ...
    if len(tmplist)>2 : 
        i2 =plot_comp(df_Cl2p, df_Cl2pT,4,'Cl 2p',MinMaxCl2p,MinCl2p,MaxCl2p,i2)
        df_Cl2p.to_csv(AnaFolder+'Data_CSV\comp_Cl2p.csv', sep='\t')
        if plotline == True:
            if UseOffsetComp == True: LabellingNumber = 0
            plot_line(DEF_lines, df_Cl2p[tmplist[0]], MinMaxCl2p, MinCl2p, MaxCl2p, df_Cl2p[tmplist[2*LabellingNumber+1]], 1.12)
    tmplist = list(df_Cl2ps)
    if len(tmplist)>2 : 
        if IsShirley:
            i2s =plot_comp(df_Cl2ps, df_Cl2psT,5,'Cl 2p',MinMaxCl2p,MinCl2p,MaxCl2p,i2s)
            df_Cl2ps.to_csv(AnaFolder+'Data_CSV\comp_Cl2pS.csv', sep='\t')
            if plotline == True:
                plot_line(DEF_lines, df_Cl2ps[tmplist[0]], MinMaxCl2p, MinCl2p, MaxCl2p, df_Cl2ps[tmplist[2*LabellingNumber+1]], 1.12)
    #Cl2p---------------------------------------------------    
    ...
    if len(tmplist)>2 :
        i2 =plot_comp(df_Cr2p, df_Cr2pT,4,'Cr 2p',MinMaxCr2p,MinCr2p,MaxCr2p,i2)
    ...
    if len(tmplist)>2 : 
        if IsShirley:
            i2s =plot_comp(df_Cr2ps, df_Cr2psT,5,'Cr 2p',MinMaxCr2p,MinCr2p,MaxCr2p,i2s)
    ...
    if LabellingAutoHeight == True: 
        ...
        for i in range(0, int(len(TmpDef1.columns)/2)):
            ...
            #print('cccccc   ',max(TmpDef1[tmplist[2*i+1]].iloc[ix_Max:ix_Min]))
            if ix_Min == ix_Max : I_maxCOMP = max(TmpDef1[tmplist[2*i+1]])
    ...
    if len(tmplist)>2 : 
        i2 =plot_comp(df_S2p, df_S2pT,4,'S 2p',MinMaxS2p,MinS2p,MaxS2p,i2)
    ...
    if len(tmplist)>2 : 
        if IsShirley: 
            i2s =plot_comp(df_S2ps, df_S2psT,5,'S 2p',MinMaxS2p,MinS2p,MaxS2p,i2s)
    ...
    if len(tmplist)>2 : 
        i2 =plot_comp(df_Al2p, df_Al2pT,4,'Al 2p',MinMaxAl2p,MinAl2p,MaxAl2p,i2)
    ...
    if len(tmplist)>2 : 
        if IsShirley: 
            i2s =plot_comp(df_Al2ps, df_Al2psT,5,'Al 2p',MinMaxAl2p,MinAl2p,MaxAl2p,i2s)
    ...
    if len(tmplist)>2 : 
        i2 =plot_comp(df_Si2p, df_Si2pT,4,'Si 2p',MinMaxSi2p,MinSi2p,MaxSi2p,i2)
    ...
    if len(tmplist)>2 : 
        if IsShirley:
            i2s =plot_comp(df_Si2ps, df_Si2psT,5,'Si 2p',MinMaxSi2p,MinSi2p,MaxSi2p,i2s)
    ...
    if len(tmplist)>2 : 
        i2 =plot_comp(df_Zn2p, df_Zn2pT,4,'Zn 2p',MinMaxZn2p, MinZn2p, MaxZn2p,i2)
        df_Zn2p.to_csv(AnaFolder+'Data_CSV\comp_Zn2p.csv', sep='\t')
        if plotline == True:
            if UseOffsetComp == True: LabellingNumber = 0
            plot_line(DEF_lines, df_Zn2p[tmplist[0]], MinMaxZn2p, MinZn2p, MaxZn2p, df_Zn2p[tmplist[2*LabellingNumber+1]], 1.12)
    tmplist = list(df_Zn2ps)
    if len(tmplist)>2 : 
        if IsShirley:
            i2s =plot_comp(df_Zn2ps, df_Zn2psT,5,'Zn 2p',MinMaxZn2p, MinZn2p, MaxZn2p,i2s)
            df_Zn2ps.to_csv(AnaFolder+'Data_CSV\comp_Zn2pS.csv', sep='\t')
            if plotline == True:
                plot_line(DEF_lines, df_Zn2ps[tmplist[0]], MinMaxZn2p, MinZn2p, MaxZn2p, df_Zn2ps[tmplist[2*LabellingNumber+1]], 1.12)
    #Zn2p---------------------------------------------------
    ...
    if len(tmplist)>2 : 
        i2 =plot_comp(df_Mn3s, df_Mn3sT,4,'Mn 3s',MinMaxMn3s,MinMn3s,MaxMn3s,i2)
        df_Mn3s.to_csv(AnaFolder+'Data_CSV\comp_Mn3s.csv', sep='\t')
        if plotline == True:
            if UseOffsetComp == True: LabellingNumber = 0
            plot_line(DEF_lines, df_Mn3s[tmplist[0]], MinMaxMn3s, MinMn3s, MaxMn3s, df_Mn3s[tmplist[2*LabellingNumber+1]], 1.12)
    tmplist = list(df_Mn3ss)
    if len(tmplist)>2 : 
        if IsShirley: 
            i2s =plot_comp(df_Mn3ss, df_Mn3ssT,5,'Mn 3s',MinMaxMn3s,MinMn3s,MaxMn3s,i2s)
            df_Mn3ss.to_csv(AnaFolder+'Data_CSV\comp_Mn3sS.csv', sep='\t')
            if plotline == True:
                plot_line(DEF_lines, df_Mn3ss[tmplist[0]], MinMaxMn3s, MinMn3s, MaxMn3s, df_Mn3ss[tmplist[2*LabellingNumber+1]], 1.12)
    #Mn3s---------------------------------------------------
    ...
    if len(tmplist)>2 : 
        i2 =plot_comp(df_F1s, df_F1sT,4,'F 1s',MinMaxF1s,MinF1s,MaxF1s,i2)
    ...
    if len(tmplist)>2 : 
        if IsShirley:
            i2s =plot_comp(df_F1s, df_F1ssT,5,'F 1s',MinMaxF1s,MinF1s,MaxF1s,i2s)
    ...
    if len(tmplist)>2 : 
        i2 =plot_comp(df_Li1s, df_Li1sT,4,'Li 1s',MinMaxLi1s,MinLi1s,MaxLi1s,i2)
        df_Li1s.to_csv(AnaFolder+'Data_CSV\comp_Li1s.csv', sep='\t')
        if plotline == True:
            if UseOffsetComp == True: LabellingNumber = 0
            plot_line(DEF_lines, df_Li1s[tmplist[0]], MinMaxLi1s, MinLi1s, MaxLi1s, df_Li1s[tmplist[2*LabellingNumber+1]], 1.12)
    tmplist = list(df_Li1ss)
    if len(tmplist)>2 : 
        if IsShirley:
            i2s =plot_comp(df_Li1s, df_Li1ssT,5,'Li 1s',MinMaxLi1s,MinLi1s,MaxLi1s,i2s)
            df_Li1ss.to_csv(AnaFolder+'Data_CSV\comp_Li1sS.csv', sep='\t')
            if plotline == True:
                plot_line(DEF_lines, df_Li1ss[tmplist[0]], MinMaxLi1s, MinLi1s, MaxLi1s, df_Li1ss[tmplist[2*LabellingNumber+1]], 1.12)
    #Li1s---------------------------------------------------
    ...
    if len(tmplist)>2 : 
        i2 =plot_comp(df_Na1s, df_Na1sT,4,'Na 1s',MinMaxNa1s,MinNa1s,MaxNa1s,i2)
    ...
    if len(tmplist)>2 : 
        if IsShirley:
            i2s =plot_comp(df_Na1s, df_Na1ssT,5,'Na 1s',MinMaxNa1s,MinNa1s,MaxNa1s,i2s)
    ...
    if len(tmplist)>2 : 
        i2 =plot_comp(df_N1s, df_N1sT,4,'N 1s',MinMaxN1s,MinN1s,MaxN1s,i2)
    ...
    if len(tmplist)>2 : 
        if IsShirley:
            i2s =plot_comp(df_N1s, df_N1ssT,5,'N 1s',MinMaxN1s,MinN1s,MaxN1s,i2s)
    ...
    if len(tmplist)>2 : 
        i2 =plot_comp(df_Y3d, df_Y3dT,4,'Y 3d',MinMaxY3d, MinY3d, MaxY3d,i2)
        df_Y3d.to_csv(AnaFolder+'Data_CSV\comp_Y3d.csv', sep='\t')
        if plotline == True:
            if UseOffsetComp == True: LabellingNumber = 0
            plot_line(DEF_lines, df_Y3d[tmplist[0]], MinMaxY3d, MinY3d, MaxY3d, df_Y3d[tmplist[2*LabellingNumber+1]], 1.12)
    tmplist = list(df_Y3ds)
    if len(tmplist)>2 :
        if IsShirley:
            i2s =plot_comp(df_Y3ds, df_Y3dsT,5,'Y 3d',MinMaxY3d, MinY3d, MaxY3d,i2s)
            df_Y3ds.to_csv(AnaFolder+'Data_CSV\comp_Y3dS.csv', sep='\t')
            if plotline == True:
                plot_line(DEF_lines, df_Y3ds[tmplist[0]], MinMaxY3d, MinY3d, MaxY3d, df_Y3ds[tmplist[2*LabellingNumber+1]], 1.12)
    #Y3d---------------------------------------------------
    ...
    if IsSaveFig == True:
        ...
        if IsPresentation == True:            
            if IsComp == True:
                ...
                pic = slide.shapes.add_picture(AnaFolder+'PNGs\Survey_Comp'+'.png', 0, Cm(3.12), width=Cm(Pres_plotWidth))  
                prs.save(AnaFolder+P_Fname)            
                ...
                pic = slide.shapes.add_picture(AnaFolder+'PNGs\CoreLevel_Comp'+'.png', 0, Cm(3.12), width=Cm(Pres_plotWidth)) 
                
                blank_slide_layout = prs.slide_layouts[5]
                slide = prs.slides.add_slide(blank_slide_layout)
                shapes = slide.shapes 
                shapes.title.text = 'Comparisons: Core levels with Shirley correction'
                pic = slide.shapes.add_picture(AnaFolder+'PNGs\CoreLevel_Comp_Shirley'+'.png', 0, Cm(3.12), width=Cm(Pres_plotWidth))    
                prs.save(AnaFolder+P_Fname)
...
#InfoWin = Tk.Text(root, height=62, width=35)
#InfoWin.grid(column = 1, row = 0)
#InfoWin.insert(Tk.END, "")


tabControl = ttk.Notebook(root)          # Create Tab Control
tabSurvey = ttk.Frame(tabControl)            # Create a tab 
tabCore = ttk.Frame(tabControl)            # Create a tab 
tabCoreFitted = ttk.Frame(tabControl)
tabVal = ttk.Frame(tabControl)            # Create a tab 
tabSurveyComp = ttk.Frame(tabControl)            # Create a tab 
tabCoreComp = ttk.Frame(tabControl)            # Create a tab 
tabValComp = ttk.Frame(tabControl)            # Create a tab 
tabControl.add(tabSurvey, text='Survey')      # Add the tab
tabControl.grid(column = 1, row = 0)
tabControl.add(tabCore, text='Core Level')      # Add the tab
tabControl.grid(column = 1, row = 0)
tabControl.add(tabCoreFitted, text='Fitted Core Level')      # Add the tab
tabControl.grid(column = 1, row = 0)
tabControl.add(tabVal, text='Valence')      # Add the tab
tabControl.grid(column = 1, row = 0)
tabControl.add(tabSurveyComp, text='Comparison: Survey')      # Add the tab
tabControl.grid(column = 1, row = 0)
tabControl.add(tabCoreComp, text='Comparison: Core Level')      # Add the tab
tabControl.grid(column = 1, row = 0)
tabControl.add(tabValComp, text='Comparison: Valence')      # Add the tab
tabControl.grid(column = 1, row = 0)
...
menu.add_command(label='Exit', command=_quit)
menu.add_command(label='Run', command=_run)
#helpmenu = Tk.Menu(menu)
#menu.add_cascade(label='Help', menu=helpmenu)
#helpmenu.add_command(label='About', command=popupBonus)
...
# a tk.DrawingArea
#FigWin = plt.figure(20, figsize=(5, 5), dpi=100)
canvas = FigureCanvasTkAgg(fig, master=tabCore)
...
canvas._tkcanvas.grid(column = 1, row = 0)
canvas.draw()



canvas2 = FigureCanvasTkAgg(fig_survey2, master=tabSurvey)
...
canvas3 = FigureCanvasTkAgg(fig_multi, master=tabCoreComp)
canvas3.get_tk_widget().grid(column = 1, row = 0)
canvas3._tkcanvas.grid(column = 1, row = 0)
canvas3.draw()



canvas4 = FigureCanvasTkAgg(fig_val, master=tabVal)
canvas4.get_tk_widget().grid(column = 1, row = 0)
canvas4._tkcanvas.grid(column = 1, row = 0)
canvas4.draw()

canvas5 = FigureCanvasTkAgg(fig_survey, master=tabSurveyComp)
canvas5.get_tk_widget().grid(column = 1, row = 0)
canvas5._tkcanvas.grid(column = 1, row = 0)
canvas5.draw()

canvas6 = FigureCanvasTkAgg(fig_valence, master=tabValComp)
canvas6.get_tk_widget().grid(column = 1, row = 0)
canvas6._tkcanvas.grid(column = 1, row = 0)
canvas6.draw()

canvas7 = FigureCanvasTkAgg(fig_fit, master=tabCoreFitted)
canvas7.get_tk_widget().grid(column = 1, row = 0)
canvas7._tkcanvas.grid(column = 1, row = 0)
canvas7.draw()
