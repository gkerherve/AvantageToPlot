...
canvas = FigureCanvasTkAgg(FigWin, master=TopFrame)
canvas.draw()
...
FigWin2 = plt.figure(21, figsize=(5, 5), dpi=100)
canvas2 = FigureCanvasTkAgg(FigWin, master=TopFrame)
canvas2.draw()
    ...
    for j in range(0, len(file)):
        ...
        if isQuant1 == True:
            ...
            if ['Quantifications' in xl.sheet_names] : 
                ...
                Area_Conc = File_Quant.iloc[0:len(Peak_Name)-1,File_Quant.columns.get_loc('Atomic %')]
                
                for j_tab in range(0, len(Peak_Name)):
                    Quant_Tab1.iloc[j_tab,1] = Quant_Tab1.iloc[j_tab,1] - CarbonComp[j]+284.8
                print(Quant_Tab1)
                ...
                if Column_Num1 > 6:
                    Quant_Tab2 = File_Quant.iloc[0:len(Peak_Name)-1, File_Quant.columns.get_loc('Atomic %')+1:]
                    #print(Quant_Tab2)
        ...
        for i in range(0, len(xl.sheet_names)-1):    
            ...
            plt.ylabel("")
            plt.xlabel("Binding Energy (eV)")
            plt.xlim(max(x_BE), min(x_BE))
            plt.tight_layout()
            plt.text(0.01,.92,plotname, horizontalalignment = "left", transform=ax.transAxes, fontsize=16, fontweight='bold'  )                
            plt.plot(x_BE,y2)
            canvas.draw()
...
button = Tk.Button(master=LeftFrame, text='Quit', command=_quit)
button.pack(side=Tk.TOP)
button = Tk.Button(master=LeftFrame, text='Run', command=_run)
button.pack(side=Tk.TOP)
#button = Tk.Button(master=LeftFrame, text='View', command=_view)
#button.pack(side=Tk.TOP)


InfoWin = Tk.Text(RightFrame, height=10, width=100)
InfoWin.pack(side = Tk.LEFT)
