...
root = Tk.Tk()
root.wm_title("Avantage data ---> powerpoint")
...
def _run():
    ...
    for tmpfile in os.listdir('.'):    
        if tmpfile.endswith(".xlsx"):
            if tmpfile.startswith("ConfigXPS_10"):
...
tabControl.grid(column = 0, row = 0)
tabControl.add(tabSurveyComp, text='Survey Comp')      # Add the tab
tabControl.grid(column = 0, row = 0)
tabControl.add(tabCoreComp, text='Core Level Comp')      # Add the tab
tabControl.grid(column = 0, row = 0)
tabControl.add(tabValComp, text='Valence Comp')      # Add the tab
tabControl.grid(column = 0, row = 0)
...
canvas2._tkcanvas.grid(column = 0, row = 0)
#canvas2._tkcanvas.grid_remove()
canvas2.draw()


canvas3 = FigureCanvasTkAgg(fig_multi, master=tabCoreComp)
canvas3.get_tk_widget().grid(column = 0, row = 0)
canvas3._tkcanvas.grid(column = 0, row = 0)
canvas3.draw()
#canvas3.get_tk_widget().grid_remove()
#canvas3._tkcanvas.grid_remove()

canvas4 = FigureCanvasTkAgg(fig_valence, master=tabVal)
canvas4.get_tk_widget().grid(column = 0, row = 0)
canvas4._tkcanvas.grid(column = 0, row = 0)
canvas4.draw()

canvas5 = FigureCanvasTkAgg(fig_survey, master=tabSurveyComp)
canvas5.get_tk_widget().grid(column = 0, row = 0)
canvas5._tkcanvas.grid(column = 0, row = 0)
canvas5.draw()
